...
class BrokerConnection:
    ...
    def get_updates(self, topic, partition):
        """
            Get updates from the corresponding broker
        """
...
class MessageQueue:
    ...
    def handle_replica_connection(self, replica):
        ...
        # (Ideally the Replica should register only when it is ready)
        time.sleep(3)
    ...
    def handle_broker_connection(self, broker):
        ...
        # make sure the broker gets all the topics
    ...
    def gather_updates(self):
        '''
            gathers the updates to send to Replica
        '''
    ...
    def worker(self):
        ...
        while True:
            ...
            if self.replica_connected and len(self.queries) > 0:
                try:
                    self.replica.push_updates(self.gather_updates())
                    for q in self.queries:
